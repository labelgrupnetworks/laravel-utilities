...
description: Usa esto cuando tengas que revisar uno o varios MCP Tools ya existentes en app/Mcp/Tools/ para comprobar si un cambio en el Controller/endpoint o el UseCase que envuelven (según la Tool) ha desincronizado su schema()/outputSchema() declarado — y actualizarlo. Localiza la clase dueña real (FormRequest, UseCase, Resource, DTO, adapter Genesis), compara campo a campo, y corrige el schema/shape de esa clase (nunca inventa la Tool desde cero, eso es mcp-tool-builder). Tres modos: por diff, dirigido a una Tool concreta, o auditoría completa de ApServer.
---

# Revisar y mantener sincronizados los MCP Tools en NAP Backend
...
Controller/FormRequest o el UseCase (input) y el Resource/DTO/adapter Genesis
...
"revisa las tools de cart" sin decir cuáles), pregunta antes de actuar. **No persiste
ningún fichero de checklist ni informe en el repo** — el resultado vive en la
conversación; si detecta un desfase y toca corregirlo, el fix se aplica al código real
(clase dueña o, en Tools legacy, la propia Tool), no a un documento aparte.
...
no todas las Tools están en el mismo sitio todavía (migración en curso, ver
`mcp-tool-builder`):

- **Vía atributo** (patrón actual): la Tool lleva `#[Schema(Clase::class)]` y/o
  `#[OutputSchema(Clase::class, key?, description?, many?)]`; el `schema()`/`shape()`
  real vive en `Clase` (FormRequest/Data/UseCase/Resource/DTO/clase dedicada en
  `app/Mcp/Schemas/{Dominio}/`), resuelto por `ResolvesToolSchemas`. **El fix va en
  `Clase`, nunca en la Tool.** `#[Schema]` es **repetible**: una Tool puede llevar
  `#[Schema(A::class)]` + `#[Schema(B::class)]` apilados cuando el input combina un
  FormRequest con un param/model sin dueño natural único (`ResolvesToolSchemas::schema()`
  hace `array_merge()` de cada `Clase::schema($schema)` en orden de declaración) — caso
  real: `DeliveryExcesses\UpdateTool` (`#[Schema(IdDeliveryExcessSchema::class)]` +
  `#[Schema(UpdateRequest::class)]`). Al revisar una Tool así, compara **cada** clase
  apilada contra su propia fuente (el param contra la firma real del Controller, el
  FormRequest contra sus `rules()`) — no asumas que basta con revisar una y extrapolar
  a la otra, y confirma que el orden de los atributos sigue reflejando el orden real de
  los campos si la Tool legacy ya tenía un orden establecido. `#[OutputSchema]` **no**
  es repetible (no hay precedente que lo requiera). `many: true` (requiere `key`) envuelve
  el `shape()` como
  array de objetos bajo la clave — la misma `Clase` sirve objeto único (sin `many`) y
  colección (`many: true`); al revisar, comprueba que la cardinalidad del atributo
  (`many` sí/no) coincide con si la respuesta real es una lista o un objeto único.
  Paginación compuesta (`{items:[...], data:{...}}` o `{<dominio>:[...],
  pagination:{...}}`) tampoco es limitación del atributo: `key = null` +
  `shape()` de 2 keys basta. Verifica que usa `PaginationTrait::paginationOutputSchema()`
  (Eloquent `LengthAwarePaginator`: `items`+`data{current_page,last_page,total_items}`)
  o `PaginationTrait::paginationGenesisOutputSchema()` (respuesta HTTP externa:
  `<dominio>`+`pagination{total_pages,total_items}`, enteros planos) — según de dónde
  venga el dato real. **Nunca asumas el wrapping `NumericType` en los campos de
  paginación Genesis sin comprobarlo contra la respuesta real** (`GetRestrictedUseCase::parseResult()`
  confirmó enteros planos, no `NumericType`).
- **A mano en la Tool** (legacy, aún vivo mientras no se migre): la Tool define
  `public function schema(JsonSchema $schema): array` / `outputSchema(...)` con el
  array construido dentro de sí misma, típicamente con
  `App\Mcp\Core\Tools\Concerns\BuildsCommonSchemas`
  (`$this->numericSchema()`/`$this->translateSchema()`/`$this->paginationSchema()`).
  **El fix va en la propia Tool** — no es obligatorio migrarla al patrón de atributos
  en el mismo pase (indícalo como oportunidad, pero corrige el desfase real primero;
  migrar es una tarea aparte que el usuario puede pedir explícitamente).
...
- **Auditoría completa** (todo `ApServer::$tools`) → Paso 3.
...
1. **Identifica qué cambió**: `git diff --stat` sobre Controllers
   (`app/Http/Controllers/`), FormRequests (`app/Http/Requests/`, `src/Nap/**/DTO/`),
   UseCases (`src/Nap/**/Actions/`), o Resources/DTOs de salida.
2. **Localiza las Tools afectadas**:
   - Si el Controller/método cambió: grep del Controller dentro de `endpoint():
     EndpointDTO` en `app/Mcp/Tools/`.
   - Si cambió una clase referenciada por atributo: grep sobre
     `#[Schema(NombreClase::class)]` / `#[OutputSchema(NombreClase::class` en
     `app/Mcp/Tools/`.
...
4. **Si hay desfase**: corrígelo en el sitio correcto (ver "Dónde vive el schema
   ...
   para output, incluida la tabla de casts Genesis si aplica) — nunca adivines el
   nuevo campo, repite la misma evidencia que exigiría construirlo desde cero.
...
1. **Localiza cada Tool del alcance dado** (`codegraph_search`/grep por
   ...
   es "todo `Carts/`" o similar) y determina para cada una si envuelve un endpoint o un
   UseCase (mira si extiende `ControllerTool`/`DataObjectControllerTool` o
   `UseCaseTool`).
2. **Para cada Tool, localiza su clase dueña real de hoy**:
   - Endpoint: el Controller+método vigente (via `EndpointDTO`) y su FormRequest (si
   ...
   - Output (ambos caminos): el Resource/DTO/clase de schema dedicada que produce la
     ...
     incluida la tabla de casts Genesis del Paso 3.2 de `mcp-tool-builder` si el
     Controller/UseCase llama a GenesisSDK. Si ese Adapter delega un campo a otro
     Adapter (`OtroAdapter::make()`/`::collection()`), comprueba que la clase de
     schema replica esa composición (una clase por Adapter, compuestas entre sí) en
     vez de una única clase monolítica aplanando los dos niveles — si encuentras una
     clase monolítica cubriendo un Adapter padre + hijo, es un hallazgo a reportar
     aunque los campos coincidan uno a uno (afecta a reutilización, no a corrección
     puntual). Ver Paso 3.2 punto 5 de `mcp-tool-builder`.
...
1. Lee `app/Mcp/Servers/ApServer.php` → array `$tools` (fuente única de verdad de qué
   Tools están activas; hoy ~144 entradas).
...
| Tool | Estado | Detalle |
|---|---|---|
| `carts_show` | sin cambios | `CartDataSchema::shape()` cubre 100% de `CartDataReadonly`/`CartItemDataReadonly` actuales |
| `vaccines_laboratories` | desfase detectado | `LaboratoriesSchema::shape()` no declara `name`, presente en `LaboratoryAdapter::toArray()` (`vendor/labelgrup/genesis-sdk/src/Adapters/Vaccines/LaboratoryAdapter.php:24`) |
| `deliveryexcesses_list` | legacy, sin desfase | `outputSchema()` a mano + `$this->paginationSchema()`, pero sigue cubriendo el Resource actual |
...
Y, si la Tool corregida modifica datos, pruébala contra la pharmacy `S77776`, nunca
contra datos reales, salvo que la tarea especifique otra explícitamente.
...
- [ ] Para cada Tool revisada: identificado endpoint vs UseCase, y la clase dueña real
      de hoy (Controller+FormRequest, o UseCase+constructor; Resource/DTO/adapter
      Genesis para el output).
...
- [ ] Si hay desfase: corregido en el sitio correcto, con el mismo rigor de
      `mcp-tool-builder` (tipos, modificadores, nullability, tabla de casts Genesis si
      aplica) — no solo el campo que faltaba, todo el shape re-verificado.
...
- [ ] Tras cualquier fix: `vendor/bin/pint --dirty --format agent` y `php -l`
      limpios; pharmacy `S77776` si la Tool modifica datos.
